...
- ``os-kea-ddns`` 2.7 — Kea DHCP Dynamic DNS support
- ``os-hello_world`` 1.1
...
- ``${ABI}`` is a pkg built-in variable (e.g., ``FreeBSD:15:amd64``) resolved at runtime.
- ``${SERIES}`` is the OPNsense series (e.g., ``26.7``) from ``opnsense-version -a``.
